...
vector* load_data(FILE* f, int* n, int* d) {
    ...
    *n = 0; *d = 0;
    while ((res = fscanf(f, "%lf%c", &val, &c)) >= 1) {
        ...
        if(!buf) { free_vector_list(head); return NULL; }
        buf[col++] = val;
...
/* Helper function to free the workspace matrices used in symnmf */
void free_symnmf_workspace(double **H_old, double **WH, double **Ht, double **HtH, double **HHtH, int n, int k) {
    if (H_old) free_matrix(H_old, n);
    if (WH) free_matrix(WH, n);
    if (Ht) free_matrix(Ht, k);
...
void symnmf(double** H, double** W, int n, int k){
    int iter;
    double diff;
    double **H_old, **WH, **Ht, **HtH, **HHtH;

    /* Allocate each intermediate matrix separately so we can free earlier ones on failure */
    ...
    if (!H_old || !WH || !Ht || !HtH || !HHtH) {
        free_symnmf_workspace(H_old, WH, Ht, HtH, HHtH, n, k);
        printf("%s\n", ERROR_MSG);
        return;
    }

    for (iter = 0; iter < MAX_ITER; iter++) {
        copy_matrix_values(H_old, H, n, k);
