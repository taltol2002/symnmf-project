...
def parse_k(argv):
    """
    Parses and validates the K argument from command line.
    K must be a positive integer. Exits with an error message if invalid.
    """
    try:
        k = int(argv[1])
        if k <= 0:
...
def parse_iter(argv):
    """
    Parses and validates the optional max_iter argument from command line.
    If not provided, returns the default value MAX_ITER_DEFAULT.
    max_iter must be an integer in the range (1, 800). Exits with an error if invalid.
    """
    if len(argv) <= 2:
        return MAX_ITER_DEFAULT
    try:
        max_iter = int(argv[2])
        # Validate the range: must be strictly between 1 and 800
...
def parse_args():
    """
    Parses all command line arguments and returns (k, max_iter).
    Validates argument count first, then delegates to parse_k and parse_iter.
    Exits with an error message if the argument count is wrong.
    """
